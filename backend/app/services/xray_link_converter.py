...
def convert_uri_json(host="127.0.0.1", port=10809, socksport=10808, uri=None):
    ...
    if Vless_Reality_checker(uri):
        return convert_uri_reality_json(host, port, socksport, uri)
    elif vless_ws_checker(uri):
        return convert_uri_vless_ws_json(host, port, socksport, uri)
    elif vless_tcp_checker(uri):
        return convert_uri_vless_tcp_json(host, port, socksport, uri)
    elif vmess_ws_checker(uri):
        return convert_uri_vmess_ws_json(host, port, socksport, uri)
    elif vmess_tcp_checker(uri):
        return convert_uri_vmess_tcp_json(host, port, socksport, uri)
    elif trojan_Reality_checker(uri):
        return convert_uri_trojan_reality_json(host, port, socksport, uri)
    elif trojan_ws_checker(uri):
        return convert_uri_trojan_ws_json(host, port, socksport, uri)
    elif trojan_tcp_checker(uri):
        return convert_uri_trojan_tcp_json(host, port, socksport, uri)

    
    raise ValueError("Unsupported or invalid proxy URI")
